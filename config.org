...
#+begin_src emacs-lisp
  (use-package emacs
    ...
    ;;; functions (migrate to user-lisp-directory whenever Emacs 31 is released) ;;;
    ...
    ("C-M-s-k" . christener/kill-buffer)
    ;; alternative keybindings for resizing
    ("s--" . shrink-window)
    ("s-+" . enlarge-window)
    ("C-s--" . shrink-window-horizontally)
    ("C-s-+" . enlarge-window-horizontally)
    ;; alt kbd for previous/next buffer
    ("C-M-s-h" . previous-buffer)
    ("C-M-s-l" . next-buffer)
    ;; alt window management
    ("M-s-<return>" . (lambda () (interactive) (progn (split-window-right) (other-window 1))))
    ("C-s-<return>" . (lambda () (interactive) (progn (split-window-below) (other-window 1))))
    ("s-c" . delete-window)
...
* Python
#+begin_src emacs-lisp
  ;; to avoid dumb terminal warning
  (setq python-shell-process-environment '("PYTHON_BASIC_REPL=1"))

  (use-package pythonic
    :ensure t
    :config
    (defun run-venv ()
      "Activate the .venv in the current directory (if available) and start a Python REPL."
      (interactive)
      (let ((venv-dir (file-name-concat default-directory ".venv")))
        (if (file-directory-p venv-dir)
  	    (progn
  	      (pythonic-activate (file-name-concat default-directory ".venv"))
  	      (run-python)
  	      (switch-to-buffer-other-window "*Python*"))
  	(message "Cannot find virtualenv in the current directory")
  	)))
    )
...
#+begin_src emacs-lisp
  (with-eval-after-load 'dired
    (defun christener/dired-mouse-find-file (event)
    ...
    (keymap-set dired-mode-map "<mouse-2>" 'christener/dired-mouse-find-file)
