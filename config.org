...
#+begin_src emacs-lisp
  ...
  (use-package emacs
    ...
    (unless (display-graphic-p)
        ;; está bueno ver la bateria y la hora en tty
      (display-time-mode t)
    ...
    :custom
    (auth-source-save-behavior nil) ; auth-source never saves
    (line-number-mode t)
    (column-number-mode t)
    (mode-line-position-column-line-format '("%l:%C"))
    
    :bind
    ("C-x C-a" . auto-fill-mode)
    ("C-x M-x" . christener/open-config-file)
    ("s-r" . window-rotate-split)
    ("s--" . shrink-window)
    ("s-+" . enlarge-window)
